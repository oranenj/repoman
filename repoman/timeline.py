...
class Timeline:
    ...
    def __init__(self, name, source, destination):
        ...
        self.logger.info('configuring timeline [{0}] from source [{1}] into destination [{2}]'.format(
            name, source, destination))

        if not isalnum(name, '-_.'):
            raise Exception(
                'name string must only consist of alphanumeric characters, dots, underscores and dashes')

        if not os.path.isdir(source):
            raise Exception('source is not a valid directory. If it exists, check that permissions allow access')
        ...
        # the maximum amount of snapshots that can be made from the source
        # directory
        ...
        # new snapshots appended to the end, old snapshots removed from the
        # beginning
        self._lsnapshots = []
        ...
        self._cfgfile_diff = os.path.join(
            self._destination, self._cfgfile_diff_ext)
        ...
        if os.path.exists(self._datafile):
            ...
            if (name != self._name
                ...
                raise Exception(
                    'inconsistencies found loading class state from metadata file')

        # initialize options required for repositories
        self._initialize_repository_options()

        # generate default config file in case none exists, otherwise load
    ...
    def __str__(self):
        ...
        tldata = {'name': self._name, 'source': self._source, 'path': self._destination,
                  'max_snapshots': self.get_max_snapshots(), 'excludes': self.get_excludes()}
        r = """Timeline: {name} @ {path}
Source: {source}
Max Snapshots: {max_snapshots}
Excludes: {excludes}""".format(**tldata)
        return r
    ...
    def print_links(self):
        ...
        for k, v in sorted(self._links.items()):
            ...
            if snap:
                date = snap['created'].strftime("%Y.%m.%d-%H%M%S")

            print("{0:<20} {1:<20} {2:<20}".format(k, v['snapshot'], date))
    ...
    def set_excludes(self, excludes):
        ...
        if isinstance(excludes, str):
            ...
            raise Exception(
                'excludes value must be a colon-separated string or a list')
        ...
        for i in excludes_clean:
            if i[0] == '/' or i[:1] == '..' or i == '.' or i == '*':
                raise Exception(
                    'excludes value must only contain relative paths')
            exclude_path = os.path.join(self._source, i)
            if not os.path.exists(exclude_path):
                raise Exception(
                    'invalid exclude path [{0}]'.format(exclude_path))
    ...
    def freeze(self, user='root'):
        ...
        if self._frozen:
            raise Exception(
                'timeline has already been frozen by user [{0}]'.format(self._frozen))
    ...
    def unfreeze(self, user='root'):
        ...
        if self._frozen:
            self.logger.info(
                'timeline previously frozen by user [{0}] has been unfrozen by [{0}]'.format(self._frozen, user))
            self._frozen = None
        else:
    ...
    def _rm_snapshot(self, snapshot, deleted_snapshot):
        if deleted_snapshot.has_key('diff_log_file'):
            ...
            subprocess.check_call(
                ['rm', '-f', deleted_snapshot['diff_log_file']])
    ...
    def _save_state(self):
        ...
        pickle.dump(d, fh)
        self.logger.debug(
            'current state saved into [{0}]'.format(self._datafile))
...
# =============================================================================================================================""", '' )
        ...
        cfg.set('ADVANCED', 'copy_files_recursive',
                ':'.join(self._copy_files_recursive))
        cfg.set('ADVANCED', 'copy_dirs_recursive',
    ...
    def _initialize_repository_options(self):
        ...
        if distro == 'redhat':
            ...
            # 'repoview' left out due to size
            self._copy_dirs_recursive = ['repodata']
            self._copy_files_recursive = []
        else:
            # 'i18n' left out due to size
            self._copy_dirs_recursive = ['binary-*', 'source']
            self._copy_files_recursive = [
                'Release', 'Release.gpg', 'InRelease', 'Contents-*.gz', 'Index']
        ...
        for i in os.listdir(source_path):
            ...
            for e in self._excludes:
                ...
                if source_obj == exclude_obj:
                    self.logger.debug(
                        'excluding (skipping) object [{0}]'.format(exclude_obj))
                    break
            else:
        ...
        for e in self._excludes:
            if '/' in e:
                ...
                if os.path.exists(exclude_obj) or os.path.islink(exclude_obj):
                    self.logger.debug(
                        'excluding (deleting) object [{0}]'.format(exclude_obj))
                    subprocess.check_call(['rm', '-rf', exclude_obj])
                else:
                    self.logger.warning(
                        'trying to exclude (delete) unexisting object [{0}]'.format(exclude_obj))
    ...
    def _snapshot_find_and_copy_objects(self, source_path, snapshot_path):
        ...
        if self._copy_dirs_recursive:
            ...
            # e.g. find /tmp/foo -type d -name repodata -o -name repoview -o
            # -name bar
            find_cmd = ['find', snapshot_path, '-type',
                        'd', '-name', self._copy_dirs_recursive[0]]
            for i in self._copy_dirs_recursive[1:]:
                find_cmd.extend(['-o', '-name', i])

            # FIXME cannot use this because python version is too old...
            #cdirs = subprocess.check_output( find_cmd ).split()
            cdirs = subprocess.Popen(
                find_cmd, stdout=subprocess.PIPE).communicate()[0].split()
            for cdir in cdirs:
                subprocess.check_call(['rm', '-rf', cdir])
                rel_path = os.path.relpath(cdir, snapshot_path)
                self.logger.debug('copying directory [{0}] to [{1}]'.format(
                    os.path.join(source_path, rel_path), cdir))
                subprocess.check_call(
                    ['cp', '-a', os.path.join(source_path, rel_path), cdir])

        if self._copy_files_recursive:
            # generate a find cmd with list of files to be copied
            # e.g. find /tmp/foo -type f -name Release -o -name Release.gpg ...
            find_cmd = ['find', snapshot_path, '-type',
                        'f', '-name', self._copy_files_recursive[0]]
            for i in self._copy_files_recursive[1:]:
                find_cmd.extend(['-o', '-name', i])

            # FIXME cannot use this because python version is too old...
            #cfiles = subprocess.check_output( find_cmd ).split()
            cfiles = subprocess.Popen(
                find_cmd, stdout=subprocess.PIPE).communicate()[0].split()
            for cfile in cfiles:
                subprocess.check_call(['rm', '-f', cfile])
                rel_path = os.path.relpath(cfile, snapshot_path)
                self.logger.debug('copying file [{0}] to [{1}]'.format(
                    os.path.join(source_path, rel_path), cfile))
                subprocess.check_call(
                    ['cp', '-a', os.path.join(source_path, rel_path), cfile])
    ...
    def _snapshot_generate_diff_report(self):
        ...
        if len(self._lsnapshots) > 1 and self._diff_log_path:
            ...
            self.logger.info(
                'generating diff report from snapshots [{0}] -> [{1}]'.format(current_snapshot, previous_snapshot))
            ...
            cmd = ['diff', '-r', '-q', '-X', self._cfgfile_diff, self._snapshots[current_snapshot]
                   ['path'], self._snapshots[previous_snapshot]['path']]
            stdout_file = '{0}__{1}__{2}{3}'.format(os.path.join(
                self._diff_log_path, self._name), current_snapshot, previous_snapshot, self._difflog_ext)
            ...
            self.logger.debug(
                'generated diff log file [{0}]'.format(stdout_file))
    ...
    def create_named_snapshot(self, snapshot, source_snapshot=None):
        ...
        if source_snapshot:
            self.logger.info(
                'using source snapshot [{0}]'.format(source_snapshot))
        ...
        if not isalnum(snapshot, '-_.'):
            raise Exception(
                'snapshot name must only consist of alpha-numeric characters, dots, underscores and dashes')

        # create new snapshot
        snapshot_path = os.path.join(self._destination, snapshot)
        if os.path.exists(snapshot_path):
            raise Exception("Snapshot [{0}] destination already exists: {}".format(snapshot, snapshot_path))

        self._snapshots[snapshot] = {
            'created': datetime.now(), 'path': snapshot_path, 'links': []}
        self.save()
    ...
    def _snapshot_is_named(self, snapshot):
        return (snapshot not in self._lsnapshots)
    ...
    def create_snapshot(self, random_sleep_before_snapshot=None, sleep_after_snapshot=None):
        ...
        if random_sleep_before_snapshot:
            ...
            self.logger.info(
                'sleeping [{0}] seconds before taking a new snapshot'.format(sleep_time))
        ...
        if os.path.exists(os.path.join(self._destination, snapshot)):
            raise Exception('snapshot [{0}] destination directory already exists!'.format(snapshot))

        # create new snapshot
        snapshot_path = os.path.join(self._destination, snapshot)
        self._snapshots[snapshot] = {
            'created': now, 'path': snapshot_path, 'links': []}
    ...
    def delete_snapshot(self, snapshot, skip_linked=False):
        ...
        self.save()
        self._rm_snapshot(snapshot, deleted_snapshot)
        self.logger.debug('deleted snapshot [{0}] [{1}]'.format(
    ...
    def create_link(self, link, snapshot=None, max_offset=0, warn_before_max_offset=0):
        ...
        self.logger.info(
            'creating new link [{0}] to snapshot [{1}]'.format(link, snapshot))

        if not isalnum(link, '-_.'):
            raise Exception(
                'link name must only consist of alpha-numeric characters, dots, underscores and dashes')
        ...
        if max_offset:
            ...
            if self._get_snapshot_offset(snapshot) > max_offset:
                self.logger.warning('creating link to snapshot with offset [{0}] which already lies beyond max_offset [{1}]!'.format(
                    self._get_snapshot_offset(snapshot), max_offset))

        link_path = os.path.join(self._destination, link)
        self._links[link] = {'created': datetime.now(), 'snapshot': snapshot, 'path': link_path,
                             'max_offset': max_offset, 'warn_before_max_offset': warn_before_max_offset}
        ...
        self.logger.debug(
            'created new link [{0}] to snapshot [{1}]'.format(link, snapshot))
    ...
    def delete_link(self, link):
        ...
        self.logger.debug(
            'deleted link [{0}] [{1}]'.format(link, deleted_link))
    ...
    def update_link(self, link, snapshot=None):
        ...
        self.logger.info(
            'updating link [{0}] to snapshot [{1}]'.format(link, snapshot))
        ...
        if snapshot == self._links[link]['snapshot']:
            self.logger.warning(
                'link [{0}] already points to snapshot [{1}]!'.format(link, snapshot))

        if self._links[link]['max_offset']:
            if self._get_snapshot_offset(snapshot) > self._links[link]['max_offset']:
                self.logger.warning('updating link to snapshot with offset [{0}] which lies beyond specified max_offset [{1}]!'.format(
        ...
        # make changes in the file system
        subprocess.check_call(
            ['ln', '-snf', snapshot, self._links[link]['path']])

        self.logger.info(
            'updated link [{0}] to snapshot [{1}]'.format(link, snapshot))
    ...
    def _get_neighbour_snapshot(self, snapshot):
        ...
        if snapshot not in self._lsnapshots:
            raise Exception(
                "Can't iterate from named snapshots. This should not happen")

        snapshot_index = self._lsnapshots.index(snapshot)

        # if we only have a single snapshot left we return it as it's
        # neighbour...
    ...
    def _get_snapshot_offset(self, snapshot):
        ...
        if self._snapshot_is_named(snapshot):
            self.logger.warning(
                "Named snapshots do not have offsets, so max_offset has no effect")
            return 0
    ...
    def _valid_snapshot(self, snapshot, fail_on_disk_check=True):
        ...
        if not os.path.exists(self._snapshots[snapshot]['path']):
            msg = 'snapshot [{0}] not found!'.format(
                self._snapshots[snapshot]['path'])
